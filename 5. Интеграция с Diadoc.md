...
> Связанные материалы: [PROJECT.md](1.%20Описание%20проекта.md) · [INTEGRATION_AVANKOR.md](6.%20Интеграция%20с%20Аванкор.md) · [INTEGRATION_SPEC_DEP.md](8.%20Интеграция%20со%20Спецдепозитарием.md) · [INTEGRATION_BANK_CLIENT.md](7.%20Интеграция%20с%20Банк-клиентом.md)
...
| **Исходящие** | Договора, Счета и акты арендаторам | Ручная работа в Diadoc / почта           | Отправка из Veles после подготовки (этап 2+)         |
...
|--------|----------|-----------|
| **Локальная КЭП** | CryptoPro + сертификат на сервере/рабочей станции | Сложно в Streamlit-прототипе на Linux; в prod — на стороне FastAPI-сервера или CloudSign |
...
> **Прототип:** UI и вызовы клиента — в Streamlit (`app/`). **Продакшен:** React вызывает FastAPI, интеграция — в `integrations/diadoc/`.

```mermaid
flowchart TB
    subgraph veles [Veles]
        UI["UI: React (prod) / Streamlit (прототип)"]
        API[FastAPI — prod]
    ...
    API --> SVC
    SVC --> DB
    SVC -->|GET events, content| DAPI
    SVC -->|POST PostMessage| DAPI
    SVC -.->|sign| CS
```
...
| Компонент | Выбор |
|-----------|--------|
| HTTP | `httpx` (async в FastAPI prod; sync в Streamlit-прототипе) или `requests` |
...
- [PROJECT.md](1.%20Описание%20проекта.md)
- [INTEGRATION_AVANKOR.md](6.%20Интеграция%20с%20Аванкор.md)
- [INTEGRATION_SPEC_DEP.md](8.%20Интеграция%20со%20Спецдепозитарием.md)
- [INTEGRATION_BANK_CLIENT.md](7.%20Интеграция%20с%20Банк-клиентом.md)
